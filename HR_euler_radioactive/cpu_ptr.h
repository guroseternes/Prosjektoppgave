...
public:
	...
	float printToFile(FILE* filePtr, bool withHeader = false, bool withBorder = false);
...
float cpu_ptr_2D::printToFile(FILE* filePtr, bool withHeader, bool withBorder){
	float pollCounter = 0;
	...
	if (not withBorder){	
		...
		for (int j=0; j<nx; j++){
			for (int i=0; i<nx; i++){
				...
				pollCounter += this->operator()(i-border, j-border);
			}
		}
	}else{
		if (withHeader){
