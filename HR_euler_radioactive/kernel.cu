	...
	inline __device__ void RK4(float& x, float& y, float dt, float lambda){
		float k1 = -dt*x*lambda;
		float i1 = -k1;
		float k2 = -dt*(x+k1*0.5)*lambda;
		float i2 = -k2;
		float k3 = -dt*(x+k2*0.5)*lambda;
		float i3 = -k3;
		float k4 = -dt*(x+k3)*lambda;
		float i4 = -k4;
		y = y + (1.0f/6.0f)*(i1+2*i2+2*i3+i4);
		x = x + (1.0f/6.0f)*(k1+2*k2+2*k3+k4);
	}

	__global__ void RKKernel(int step){ 
		
		// decay rate implemented directly at first
		float dt = rk_ctx.dt[0];
		//printf("TIME %.6f\n", dt);
		float u0,u1,u2,u3,u4,u5,r0,r1,r2,r3,r4,r5,q0,q1,q2,q3,q4,q5;
		...
		u4 = global_index(rk_ctx.U4.ptr, rk_ctx.U4.pitch, xid, yid, global_border)[0];		
		u5 = global_index(rk_ctx.U5.ptr, rk_ctx.U5.pitch, xid, yid, global_border)[0];		
		...
		r4 = global_index(rk_ctx.R4.ptr, rk_ctx.R4.pitch, xid, yid, global_border)[0];
		r5 = global_index(rk_ctx.R5.ptr, rk_ctx.R5.pitch, xid, yid, global_border)[0];
		...
		if (step == 0) {
			...
			q4 = global_index(rk_ctx.Q4.ptr, rk_ctx.Q4.pitch, xid, yid, global_border)[0];
			q5 = global_index(rk_ctx.Q5.ptr, rk_ctx.Q5.pitch, xid, yid, global_border)[0];
			...
			RK4(q4, q5, dt, rk_ctx.lambda);

/*
			float q5temp = q4*rk_ctx.lambda;
			q5temp = q4+dt*0.5*q5temp;
			q5 = q5 + dt*q5temp*rk_ctx.lambda;


			float q4temp = -q4*rk_ctx.lambda;
			q4temp = q4+dt*0.5*q4temp;
			q4 = q4 - dt*q4temp*rk_ctx.lambda;


			float q4temp = q4 + dt*(-q4*rk_ctx.lambda);
			float q5temp = q5 + dt*(q4*rk_ctx.lambda);

			q5 = q5 + dt*0.5*(q4*rk_ctx.lambda+ q4temp*rk_ctx.lambda);
			q4 = q4 + dt*0.5*(-q4*rk_ctx.lambda -q4temp*rk_ctx.lambda);
*/		
			// ODE solver, Euler first
	//		q5 = q5 + dt*(q4)*rk_ctx.lambda;
	//		q4 = q4 - dt*(q4)*rk_ctx.lambda;
		...
		global_index(rk_ctx.Q4.ptr, rk_ctx.Q4.pitch, xid, yid, global_border)[0] = q4;
		global_index(rk_ctx.Q5.ptr, rk_ctx.Q5.pitch, xid, yid, global_border)[0] = q5;
	}
	...
	inline __device__ void fluxAndLambdaFuncF(float& rho, float& U1, float& U2, float& U3, float& U4, float& U5,
			const float& gamma,
		...
		F2 = U1*v;
		F3 = u*(E+pressure);
		F4 = flux_ctx.alphaA*u*U4/rho;
		F5 = flux_ctx.alphaB*u*U5/rho;
	}

	inline __device__ void fluxAndLambdaFuncG(float& rho, float& U1, float& U2, float& U3, float& U4, float& U5,
			const float& gamma,
			float& G0, float& G1, float& G2, float& G3,
			float& G4, float& G5, float& u, float& v,float& c){
		...
		G2 = U2*v + pressure;
		G3 = v*(E+pressure);
		G4 = flux_ctx.betaA*v*U4/rho;
		G5 = flux_ctx.betaB*v*U5/rho;
	}
	...
	*/
	...
	inline __device__ void reconstructPointVal(float (&U)[6][BLOCKDIM_X][SM_BLOCKDIM_Y], float (&Ux)[6][BLOCKDIM_X][SM_BLOCKDIM_Y], float (&Uy)[6][BLOCKDIM_X][SM_BLOCKDIM_Y], unsigned int i, unsigned int j){
	...
	inline __device__ float computeFluxWest(float (&U)[6][BLOCKDIM_X][SM_BLOCKDIM_Y], float (&Ux)[6][BLOCKDIM_X][SM_BLOCKDIM_Y], unsigned int i, unsigned int j){
		
		float U0m, U1m, U2m, U3m, U4m, U5m;
		float U0p, U1p, U2p, U3p, U4p, U5p;
		float FG0p, FG1p, FG2p, FG3p, FG4p, FG5p;
		float FG0m, FG1m, FG2m, FG3m, FG4m, FG5m;
		...
		U4m = U[4][i-1][j] + Ux[4][i-1][j];
		U5m = U[5][i-1][j] + Ux[5][i-1][j];
		...
		U4p = U[4][i][j] - Ux[4][i][j];
		U5p = U[5][i][j] - Ux[5][i][j];

		fluxAndLambdaFuncF(U0p, U1p, U2p, U3p, U4p, U5p, flux_ctx.gamma, FG0p, FG1p, FG2p, FG3p,FG4p, FG5p, up, vp, cp);
		fluxAndLambdaFuncF(U0m, U1m, U2m, U3m, U4m, U5m,flux_ctx.gamma, FG0m, FG1m, FG2m, FG3m, FG4m, FG5m,um, vm, cm);
	...
	//	printf("am: %.3f ap: %.3f Ux: %.3f Um: %.3f Up: %.3f U1m: %.3f U2m %.3f U3m: %.3f U3p:%.3f gamma: %.3f up: %.3f um: %.3f vp: %.3f vm: %.3f cp: %.3f cm: %.3f\n", am,ap,Ux[0][i][j], U0m, U0p, U1m, U2m,U3m, U3p, flux_ctx.gamma, up, um, vp,vm, cp, cm);  
		...
		Ux[4][i][j] = ((ap*FG4m -am*FG4p) + ap*am*(U4p-U4m))/(ap-am);
		Ux[5][i][j] = ((ap*FG5m -am*FG5p) + ap*am*(U5p-U5m))/(ap-am);
		
		return flux_ctx.dx/fmaxf(ap, -am);
	}
		
	inline __device__ float computeFluxSouth(float (&U)[6][BLOCKDIM_X][SM_BLOCKDIM_Y],float (&Uy)[6][BLOCKDIM_X][SM_BLOCKDIM_Y], unsigned int i, unsigned int j){
		...
		U4m = U[4][i][j-1] + Uy[4][i][j-1];
		U5m = U[5][i][j-1] + Uy[5][i][j-1];
		...
		U4p = U[4][i][j] - Uy[4][i][j];
		U5p = U[5][i][j] - Uy[5][i][j];

		fluxAndLambdaFuncG(U0p, U1p, U2p, U3p, U4p, U5p, flux_ctx.gamma, FG0p, FG1p, FG2p, FG3p,FG4p, FG5p, up, vp, cp);
		fluxAndLambdaFuncG(U0m, U1m, U2m, U3m, U4m, U5m,flux_ctx.gamma, FG0m, FG1m, FG2m, FG3m, FG4m, FG5m,um, vm, cm);
		...
		Uy[4][i][j] = ((ap*FG4m -am*FG4p) + ap*am*(U4p-U4m))/(ap-am);
		Uy[5][i][j] = ((ap*FG5m -am*FG5p) + ap*am*(U5p-U5m))/(ap-am);
	...
	__global__ void fluxKernel(int step){
		...
		__shared__ float local_U[6][BLOCKDIM_X][SM_BLOCKDIM_Y];
		__shared__ float local_Ux[6][BLOCKDIM_X][SM_BLOCKDIM_Y];
		__shared__ float local_Uy[6][BLOCKDIM_X][SM_BLOCKDIM_Y];
		...
		local_U[4][i][j] = global_index(flux_ctx.U4.ptr, flux_ctx.U4.pitch, xid, yid, global_border)[0];	
		local_U[5][i][j] = global_index(flux_ctx.U5.ptr, flux_ctx.U5.pitch, xid, yid, global_border)[0];		
		...
		if (xid > -1 && xid < flux_ctx.nx && yid > -1 && yid < flux_ctx.ny){
			if ( i > 1 && i < TILEDIM_X  && j > 1 && j < TILEDIM_Y){
				...
				r4 = (local_Ux[4][i][j] - local_Ux[4][i+1][j])/dx + (local_Uy[4][i][j] - local_Uy[4][i][j+1])/dy;   
			r5 = (local_Ux[5][i][j] - local_Ux[5][i+1][j])/dx + (local_Uy[5][i][j] - local_Uy[5][i][j+1])/dy;   
			...
			global_index(flux_ctx.R4.ptr, flux_ctx.R4.pitch, xid, yid, global_border)[0] = r4;//local_Uy[0][i][j];
			global_index(flux_ctx.R5.ptr, flux_ctx.R5.pitch, xid, yid, global_border)[0] = r5;//local_Uy[0][i][j];
...
void callCollectiveSetBCOpen(dim3 grid, dim3 block, const collBCKernelArgs* arg){
	...
	callSetBCOpen(grid, block, arg->U1, arg->NX, arg->NY, arg->global_border);
        ...
        callSetBCOpen(grid, block, arg->U4, arg->NX, arg->NY, arg->global_border);
        callSetBCOpen(grid, block, arg->U5, arg->NX, arg->NY, arg->global_border);
}
