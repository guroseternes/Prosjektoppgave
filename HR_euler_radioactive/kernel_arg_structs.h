...
struct collBCKernelArgs {
        gpu_raw_ptr U0, U1, U2, U3, U4, U5; //!< input/output
...
struct RKKernelArgs {
        gpu_raw_ptr  Q0, Q1, Q2, Q3, Q4, Q5; //!< Newly calculated Q-vector (and input Q-vector for second step of RK)
        gpu_raw_ptr  U0, U1, U2, U3, U4, U5; //!< U-vector at current timestep
        gpu_raw_ptr  R0, R1, R2, R3, R4, R5; //!< Net flux in and out of cells
        float* dt;         //!< Timestep
...
struct  FluxKernelArgs {
        gpu_raw_ptr U0, U1, U2, U3, U4, U5;        //!< U vector given at cell midpoints
        gpu_raw_ptr R0, R1, R2, R3, R4, R5;        //!< Source term and net flux in and out of each cell.
	...
	float alphaA, alphaB;          // Advection constant in the x-direction 
	float betaA, betaB;
	int innerDimX;
